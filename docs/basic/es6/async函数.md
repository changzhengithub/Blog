...
1、想比较Generator函数`async`函数内置了执行器，不需要再每次去调用next。
...
任何一个`await`语句后面的 Promise 对象变为`reject`状态，`reject`的参数会被`catch`方法的回调函数接收到，整个`async`函数也会中断执行。
...
如果不想让某一个Promise影响整个函数，可以使用`try...catch` 包裹此函Promise。
...
async 想比较Promise和Generator函数更简洁和语义化。
